...
<body>
	<div class="wrap">
        <div id="content">
            <div id="content_1">
                <div id="content_1_1">
                    <div>
                        ...
                        <span>홈> <%=category%></span>
                    </div>
                </div>
                <div id="content_1_2">
                    <h2>현재 숨은 <%=category%>들</h2>
                </div>
                <%if(loginUser!=null){ %>
                <div id="content_1_3">
                 ...
                 <%for(int i=0; i<list.size(); i++){%>
                    <div class="product-list">
                        ...
                        <%} %>
                            ...
                            <span class="price" style="text-overflow:ellipsis;"><%=list.get(i).getProductPrice() %> 원</span><br>
                            <span class="nickname" style="text-overflow:ellipsis;"><%=list.get(i).getUserName() %></span>			
                        </a>
            ...
            <div id="content_3">
                <form action="<%=contextPath %>/subListSearch.pr" id="subSerch_form" method="post">
                	<input type="hidden" name="category" value="<%=category%>">
                	<input type="hidden" name="pNum" value=1>
                    <div id="subSerch_cate">
                        <select name="categorySub" id="cateSub" style="cursor: pointer;">
            ...
            <div id="content_4">
                <div class="page_wrap">
                    ...
                    <%if(pg.getCurrentPage()!=1){ %>
                       <a class="arrow prev" onclick="location.href='<%=contextPath%>/productListForm.pr?category=디지털기기&pNum=<%=pg.getCurrentPage()-1%>'"><</a>
                    <%} %>  
                    <%for(int i=pg.getStartPage(); i<=pg.getEndPage(); i++){ %>
                       <a class="active" onclick="location.href='<%=contextPath%>/productListForm.pr?category=디지털기기&pNum=<%=i%>'"><%=i%></a>
                    <%} %>
                    <%if(pg.getCurrentPage()!=pg.getMaxPage()){ %>
                       <a class="arrow next" onclick="location.href='<%=contextPath%>/productListForm.pr?category=디지털기기&pNum=<%=pg.getCurrentPage()+1%>'">></a>
                    </div>
                    <%} %> 
                 </div>
            </div>
